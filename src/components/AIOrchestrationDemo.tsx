  ...
  const renderSequentialSteps = (steps: any[]) => {
    ...
    return (
      <div className="space-y-3">
        <h4 className="font-semibold text-sm text-gray-200">Sequential Thinking Steps:</h4>
        {steps.map((step, index) => (
          <div key={index} className="border-l-4 border-blue-500 pl-4 py-2 bg-blue-50 rounded-r">
            ...
            <p className="text-sm text-gray-200 font-medium">{step.thought}</p>
            {step.reasoning && (
              <p className="text-xs text-gray-200 mt-1">{step.reasoning.substring(0, 200)}...</p>
            )}
          </div>
        ))}
      </div>
