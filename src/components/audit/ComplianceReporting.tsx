...
const ComplianceReporting: React.FC<ComplianceReportingProps> = ({
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'stable': return <Minus className="w-4 h-4 text-gray-700" />;
      default: return null;
    }
  };
  ...
  const MetricCard = ({ metric }: { metric: ComplianceMetric }) => (
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="flex items-baseline space-x-2">
        ...
        {metric.target && (
          <p className="text-sm text-gray-700">/ {metric.target}</p>
        )}
      </div>
    ...
    <div className={`space-y-6 ${className}`}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-700 mt-1">Generate and view compliance reports</p>
        </div>
        <div className="flex items-center space-x-2">
