...
export default function AdvancedFeaturesModule() {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-primary-700 mt-2">
                Master the advancedfeatures features of the NRPG CRM system
              </CardDescription>
            </div>
        ...
        <TabsList className="grid grid-cols-3 gap-4">
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('API Integration') ? (
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('Mobile Features') ? (
                <CheckCircle2 className="h-4 w-4 text-green-600" />
              ) : (
                <BookOpen className="h-4 w-4" />
              )}
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('Enterprise Features') ? (
                <CheckCircle2 className="h-4 w-4 text-green-600" />
              ) : (
                <BookOpen className="h-4 w-4" />
              )}
...
• Weather Services - BOM data for disaster tracking
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('API Integration')}
                  disabled={completedSections.includes('API Integration')}
                >
                  {completedSections.includes('API Integration') ? (
                    <>
        ...
        <TabsContent value="section-1" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">Mobile Features</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="prose prose-gray max-w-none">
                <div className="whitespace-pre-line text-gray-700 leading-relaxed">
...
• Voice-to-text job notes
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('Mobile Features')}
                  disabled={completedSections.includes('Mobile Features')}
                >
                  {completedSections.includes('Mobile Features') ? (
                    <>
                      <CheckCircle2 className="h-4 w-4 mr-2" />
...
• Dedicated account management
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('Enterprise Features')}
                  disabled={completedSections.includes('Enterprise Features')}
                >
                  {completedSections.includes('Enterprise Features') ? (
                    <>
                      <CheckCircle2 className="h-4 w-4 mr-2" />
        ...
        <Card className="border-green-200 bg-gradient-to-r from-green-50 to-green-100">
          <CardHeader>
            ...
            <CardDescription className="text-green-700">
              Congratulations! You've successfully completed the Advanced Features & Integration module.
            </CardDescription>
          </CardHeader>
          <CardContent>
