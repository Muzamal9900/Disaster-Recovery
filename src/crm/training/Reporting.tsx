    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-primary-700 mt-2">
                Master the reporting features of the NRPG CRM system
              </CardDescription>
            </div>
      ...
      <Tabs defaultValue="section-0" className="space-y-4">
        <TabsList className="grid grid-cols-3 gap-4">
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('Standard Reports') ? (
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('Custom Report Builder') ? (
                <CheckCircle2 className="h-4 w-4 text-green-600" />
              ) : (
                <BookOpen className="h-4 w-4" />
              )}
          ...
          >
            <span className="flex items-center gap-2">
              {completedSections.includes('Analytics Integration') ? (
                <CheckCircle2 className="h-4 w-4 text-green-600" />
              ) : (
                <BookOpen className="h-4 w-4" />
              )}
        ...
        <TabsContent value="section-0" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">Standard Reports</CardTitle>
            </CardHeader>
...
• Emergency Response Metrics - Response times, availability
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('Standard Reports')}
                  disabled={completedSections.includes('Standard Reports')}
                >
                  {completedSections.includes('Standard Reports') ? (
                    <>
        ...
        <TabsContent value="section-1" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">Custom Report Builder</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="prose prose-gray max-w-none">
                <div className="whitespace-pre-line text-gray-700 leading-relaxed">
...
6. Export formats (PDF, Excel, API)
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('Custom Report Builder')}
                  disabled={completedSections.includes('Custom Report Builder')}
                >
                  {completedSections.includes('Custom Report Builder') ? (
                    <>
                      <CheckCircle2 className="h-4 w-4 mr-2" />
        ...
        <TabsContent value="section-2" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">Analytics Integration</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="prose prose-gray max-w-none">
                <div className="whitespace-pre-line text-gray-700 leading-relaxed">
...
• Insurance company scorecards
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <Button
                  variant="outline"
                  onClick={() => markSectionComplete('Analytics Integration')}
                  disabled={completedSections.includes('Analytics Integration')}
                >
                  {completedSections.includes('Analytics Integration') ? (
                    <>
                      <CheckCircle2 className="h-4 w-4 mr-2" />
        ...
        <Card className="border-green-200 bg-gradient-to-r from-green-50 to-green-100">
          <CardHeader>
            ...
            <CardDescription className="text-green-700">
              Congratulations! You've successfully completed the Reporting & Analytics module.
            </CardDescription>
          </CardHeader>
          <CardContent>
